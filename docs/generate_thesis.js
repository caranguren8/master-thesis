...
const ch1 = [
  ...
  para("Second, Spain\u2019s institutional features generate analytical variation not available in other studied contexts. The 50 PR provinces range from 2-seat districts (e.g., Soria) to 37-seat megadistricts (Madrid), providing within-country variation in district magnitude that allows a direct test of whether incumbency advantages are modulated by the competitiveness of the seat allocation environment. Furthermore, Spain\u2019s party system underwent dramatic fragmentation during the sample period (2004\u20132023), with the emergence of Podemos and Ciudadanos breaking the traditional PSOE\u2013PP duopoly. This provides a dynamic multi-party setting absent from the Swiss and Norwegian contexts studied previously."),
...
const ch4 = [
  ...
  para("where Y\u1D62 is the outcome of interest, X\u1D62 is the running variable (normalized vote margin), T\u1D62 = 1(X\u1D62 \u2265 0) is the treatment indicator, and T\u1D62 \u00D7 X\u1D62 allows the slope to differ on each side of the cutoff. The parameter \u03C4 captures the local average treatment effect (LATE) of incumbency at the cutoff\u2014formally, the causal effect of a party\u2019s winning the marginal seat on the candidate\u2019s subsequent electoral outcomes, identified among observations arbitrarily close to the D\u2019Hondt allocation threshold. Under the continuity assumption (Hahn, Todd, and Van der Klaauw, 2001), this parameter is identified as the limit of the conditional expectation of Y from the right minus the left of the cutoff: \u03C4 = lim(x\u21930\u207A) E[Y|X=x] \u2212 lim(x\u21930\u207B) E[Y|X=x]."),
  para("I use a triangular kernel to weight observations inversely proportional to their distance from the cutoff. The triangular kernel is the minimax-optimal choice for local linear regression in the RDD setting, as it minimizes the maximum asymptotic mean squared error (Cheng, Fan, and Marron, 1997; as discussed in Imbens and Kalyanaraman, 2012). Three primary outcomes are considered: (i) whether the candidate reappears on a party list in the subsequent election (runs next), (ii) whether the candidate wins a seat in the subsequent election (wins next), and (iii) the candidate\u2019s list position in the subsequent election (list position next), which is observed only for candidates who do reappear and is therefore conditional on the extensive margin. Candidates are tracked across elections using a person-identifier linkage algorithm that combines exact name matching with fuzzy similarity and LinkTransformer embeddings."),
...
const ch5 = [
  ...
  para("The raw dataset covers approximately 35,000 candidate-election records across 8 elections, 50 PR provinces, and over 100 distinct party labels. After applying the D\u2019Hondt allocation algorithm to compute the closeness measure for each party-province-election combination, the data are filtered to observations within the analysis bandwidth. The person-identifier linkage algorithm successfully matched candidates across consecutive election cycles with a high degree of accuracy; manual spot-checks of a random sample of 200 linked pairs confirmed a match accuracy exceeding 95%. Cases that could not be linked with confidence\u2014due to common names, name changes, or incomplete records\u2014were flagged and excluded from the linked sample rather than being assigned probabilistic matches, ensuring that measurement error in the outcome variables is minimized."),
...
const ch8 = [
    ...
    return new Table({
      ...
      rows: [
        ...
        ...[
          ...
          ["Luechinger et al. (2024)", "Switzerland", "PR (D\u2019Hondt)", "Open", "Re-election", "Significant +"],
          ["Fiva & R\u00F8hr (2018)", "Norway", "PR (open-list)", "Open", "Re-election", "Significant +"],
  ...
  emptyPara(),
  para([new TextRun({ text: "Notes: ", italics: true, font: "Arial", size: 20 }), new TextRun({ text: "FPTP = First Past the Post; PR = Proportional Representation. All estimates use regression discontinuity designs. \u2018Significant +\u2019 indicates a statistically significant positive incumbency effect where exact magnitude is not directly comparable due to different outcome definitions. Spanish estimates use BW = 0.05 with clustered standard errors.", font: "Arial", size: 20 })]),
  emptyPara(),
...
const ch10 = [
  ...
  para("Cheng, M.-Y., Fan, J., & Marron, J. S. (1997). On automatic boundary corrections. The Annals of Statistics, 25(4), 1691\u20131708."),
  ...
  para("Fan, J., & Gijbels, I. (1996). Local Polynomial Modelling and Its Applications. Chapman and Hall."),
