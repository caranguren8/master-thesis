...
const ch4 = [
  ...
  para("In a closed-list system like Spain\u2019s, voters cannot reorder candidates within a party list. This means there is no candidate margin (Section 2.2 of Luechinger et al. does not apply). Instead, the running variable for a candidate at list position p on party j\u2019s list is simply party_margin(i = p, j). Each candidate thus receives a unique running variable reflecting how close they personally are to a seat threshold:"),
  para([
  ...
  para("The running variable is normalized by dividing by the number of eligible voters in the province, making it comparable across districts of different sizes. The cutoff is at zero; the treatment indicator equals one when the normalized margin is non-negative (i.e., the candidate holds a seat). This means that only candidates near the boundary of their party\u2019s seat allocation\u2014those at or adjacent to the marginal seat\u2014fall within a narrow bandwidth around the cutoff. Candidates safely elected at the top of the list or far down an unsuccessful list have large absolute margins and are excluded by the bandwidth restriction."),
  ...
  para("I also estimate a fuzzy RDD specification, instrumenting actual incumbency status with the treatment indicator. The first-stage relationship is near-perfect (\u03C4 \u2248 0.98), so the sharp and fuzzy estimates are nearly identical. I report both specifications for completeness."),
...
const ch5 = [
  ...
  para("The unit of analysis is the candidate-election, representing an individual candidate\u2019s participation in a specific election. As described in Section 4.2, each candidate receives a unique running variable\u2014the normalized party margin for their specific list position\u2014so that only candidates near the boundary of their party\u2019s seat allocation fall within the analysis bandwidth. Outcomes are tracked at the individual candidate level using the person-identifier linkage algorithm, enabling the analysis of personal reappearance, re-election, and list-position dynamics."),
